...
"""
def fill_image_placeholders(text : str, sources : list[str]) -> str:
    tag = Tag()
    for img in enumerate(sources):
        text = text.replace(_PLACEHOLDER_IMAGE(img[0]), tag.image(source=img[1]))
    return text
